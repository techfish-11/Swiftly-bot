...
class Growth(commands.Cog):
    ...
    async def growth(self, interaction: discord.Interaction, target: int):
        ...
        plt.scatter(join_dates, y, color='blue', label='実データ')
        plt.plot(join_dates, model.predict(X), color='red', label='回帰直線')
        plt.axhline(y=target, color='green', linestyle='--', label=f'目標: {target}')
        plt.axvline(x=target_date, color='purple', linestyle='--', label=f'予測日: {target_date.date()}')
        plt.xlabel('加入日')
        plt.ylabel('メンバー数')
        plt.title('サーバー成長予測')
        plt.legend()
