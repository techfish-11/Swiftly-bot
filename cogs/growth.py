...
class Growth(commands.Cog):
    ...
    async def growth(self, interaction: discord.Interaction, target: int, show_graph: bool = True):
        try:
            ...
            if show_graph:
                X_plot = np.linspace(
                    X[0][0], found_date.toordinal(), 200).reshape(-1, 1)
                ...
                plt.plot([datetime.fromordinal(int(x[0])) for x in X_plot],
                         y_plot, color='red', label='Prediction', linewidth=2)
                ...
                embed = discord.Embed(title="Server Growth Prediction",
                                      description=f'{target}人に達する予測日: {found_date.date()}', color=discord.Color.blue())
            ...
            embed.add_field(
                name="予測精度", value=f"{model.score(X_poly, y):.2f}", inline=True)
