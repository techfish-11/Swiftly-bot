...
class Status(commands.Cog):
    ...
    async def status(self, interaction: discord.Interaction):
        ...
        # ホストサーバーレイテンシを計測
        host_latency = await self.ping_host("192.168.1.126")
        ...
        embed.add_field(name="ホストサーバーレイテンシ", value=f"{host_latency}ms", inline=False)
    ...
    async def ping_host(self, host_ip):
        """ホストサーバーのレイテンシを計測"""
        try:
            start_time = time.time()
            async with aiohttp.ClientSession() as session:
                async with session.get(f"http://{host_ip}", timeout=3):
                    pass  # レスポンスデータは不要
            latency = round((time.time() - start_time) * 1000, 2)  # 秒 → ミリ秒
            return latency
        except aiohttp.ClientError:
